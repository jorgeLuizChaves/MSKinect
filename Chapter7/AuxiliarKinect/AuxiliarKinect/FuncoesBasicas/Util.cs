...
namespace AuxiliarKinect.FuncoesBasicas
{
    public static class Util
    {
        ...
        {
            return vetorV.X * vetorW.X + vetorV.Y * vetorW.Y + vetorV.Z * vetorW.Z;
        }

        private static double ProdutoModuloVetores(Vector4 vetorV, Vector4 vetorW)
        {
            return Math.Sqrt(Math.Pow(vetorV.X, 2) + Math.Pow(vetorV.Y, 2) + Math.Pow(vetorV.Z, 2)) *
                             Math.Sqrt(Math.Pow( vetorW.X, 2) + Math.Pow( vetorW.Y, 2) + Math.Pow(vetorW.Z, 2)
                             );
        }
